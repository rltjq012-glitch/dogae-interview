...
if st.button("🚀 면접 패키지 생성 시작"):
    ...
    if interview_type == "생기부 기반 면접":
        ...
        3. 과목명이나 주요 활동명은 반드시 **[생활과 윤리]** 처럼 볼드체로 묶어주고 도개고 기출 수준의 날카로운 꼬리질문을 포함하세요[cite: 3].
            ...
            with st.spinner("요청하신 피드백을 반영하여 처리 중입니다..."):
                ...
                if is_doc_request:
                    ...
                    if any(w in user_feedback for w in ["더", "추가", "많이", "늘려", "또"]):
                        add_instruction = "\n(주의: 사용자가 문항 추가를 요청했으므로, 독립 세트를 **최소 2세트 이상 추가로** 더 생성해 주세요!)[cite: 3]"
                    
                    feedback_prompt = f"""
                    당신은 면접 출제위원입니다. 이전 내용을 사용자의 피드백에 맞게 수정하되, 
                    심도 있는 학술적/실천적 깊이를 유지하면서 **반드시 다음 템플릿 구조와 [키워드]를 토씨 하나 틀리지 말고 유지**해 주세요[cite: 3].
                    {add_instruction}
