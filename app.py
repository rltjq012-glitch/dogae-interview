...
# 'gemini-flash-latest' / 'gemini-pro-latest'는 구글이 항상 최신 안정 모델을 가리키도록
# 유지하는 별칭이므로, 특정 버전(예: gemini-1.5-flash 등)을 하드코딩하는 것보다 안전합니다.
...
def call_gemini(prompt, api_key):
    ...
    # google-genai SDK(내부적으로 httpx 사용)가 일부 서버 환경에서
    # "'ascii' codec can't encode characters" 오류를 일으키는 문제가 있어,
    # SDK를 거치지 않고 Gemini REST API를 requests로 직접 호출합니다.
    # requests는 JSON 본문을 UTF-8 바이트로 안전하게 직렬화하므로 한글 프롬프트도 문제없이 전송됩니다.
    headers = {"Content-Type": "application/json"}
    payload_bytes = json.dumps(
        {"contents": [{"parts": [{"text": prompt}]}]}
    ).encode("utf-8")

    # 사용 가능한 모델 목록 조회 (실패해도 기본 후보군으로 계속 진행)
    available_models = []
    try:
    ...
    # 목록 조회 결과를 최신 별칭(gemini-flash-latest 등) 우선으로 정렬하고,
    # 조회에 실패했거나 결과가 비어있으면 기본 후보군을 사용합니다.
